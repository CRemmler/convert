...
to setup
  ...
  ifelse full-setup?
  [
    let users map [ [?1] -> [user-id] of ?1 ] sort patches with [is-string? user-id]
    let phases map [ [?1] -> [my-phase] of ?1 ] sort patches with [is-string? user-id]
    clear-patches
    setup-patches
    setup-intersections
...
to get-free-intersection [current-id]
  ...
  [
    ...
    [
      ...
      let my-position (word "(" my-column "," my-row ")")
      ask patch-at -2 2
      [
        set plabel-color black
        set plabel my-position
      ]
      hubnet-send current-id "Located At:" my-position
    ]
  ]
  [
