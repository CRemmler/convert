...
students-own
[
;; unique id, input by the client when they log in, to identify each student turtle
  user-id
]
...
to startup
;; CR removed "hubnet-reset". It is un-needed.
setup-vars
setup-quick-start
reset-ticks
setup
...
to reset-plot
  ...
  set num-infected-last-plotted 0
end
...
;; we only want to keep track of time once the first turtle has been infected.
  ...
  [ tick ]
;; CR removed "display". It is un-needed.
;; [ display ]
...
[
  if wander?
  [
    set heading heading + one-of [ -90 0 0 90 ]
    fd 1
    if ceiling (abs xcor) >= max-pxcor [ set heading -1 * heading ]
    if ceiling (abs ycor) >= max-pycor [ set heading 180 - heading ]
  ]
]
]
end
...
to infect-turtles [breed-to-choose]
let healthy-turtles breed-to-choose with [ not infected? ]
...
let key (code mod length shape-names)
set base-shape item key shape-names
set shape base-shape
set key code / length shape-names
set color item floor key colors
end
...
to update-sick?-monitor
  ...
  [ hubnet-send user-id "Sick?" word infected? ""
  ]
